...
BACKEND_LABEL = "BE"
FRONTEND_LABEL = "FE"
INTEGRATION_TEST_LABEL = "Integration Test"
...
def pretty(issue):
    return "{} : {} [{}/{}]".format(
        issue.key, issue.fields.summary, issue.fields.issuetype, issue.fields.status
    )
...
class Jira:
    ...
    def get_all_issues(self, user_name):
        return self.__jira.search_issues(
            f"project={self.__project} and (assignee was '{user_name}') and status = Done"
        )
    ...
    def get_current_sprint_open_issues(self):
        return self.__jira.search_issues(
            f"project={self.__project} and sprint in openSprints() and status != Done"
        )
    ...
    def get_current_sprint_subtask_open_issues(self):
        ...
        return [
            issue
            for issue in self.get_current_sprint_issues()
            if is_subtask(issue) and is_not_done(issue)
        ]
    ...
    def get_current_sprint_open_issues_without_subtask(self):
        return [
            issue
            for issue in self.get_current_sprint_issues()
            if has_no_subtask(issue) and not is_subtask(issue)
        ]
...
def fetch_all_tickets():
    ...
    for issue in issues:
        ...
        story_point = (
            issue.raw["fields"]["customfield_10013"]
            if "customfield_10013" in issue.raw["fields"]
            else "-"
        )
...
if __name__ == "__main__":
    ...
    for issue in issues:
        ...
        if has_BE_label(issue) and has_FE_label(issue):
            ...
            issue_dict["summary"] = INTEGRATION_TEST_LABEL
            be_issue = jira.get_jira().create_issue(fields=issue_dict)
            print("created Integration Test", be_issue.key, " subtask for ", issue.key)
